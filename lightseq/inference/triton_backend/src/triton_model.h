...
namespace triton { namespace backend { namespace lightseq {
/////////////
//
// ModelState
//
...
class ModelState : public BackendModel {
 ...
 public:
  ...
  TRITONSERVER_DataType GetInputDataTypeByName(std::string input_name) { return input_data_type_map_.find(input_name)->second; }
  TRITONSERVER_DataType GetOutputDataTypeByName(std::string output_name) { return output_data_type_map_.find(output_name)->second; }
...
ModelState::ValidateModelConfig()
{
  ...
  RETURN_IF_ERROR(model_type_obj.MemberAsString("string_value", &model_type_value, &model_type_length));
  ...
  RETURN_IF_ERROR(
      ModelConfig().MemberAsString("default_model_filename", &model_file_name, &file_name_len));
...
class ModelInstanceState : public BackendModelInstance {
 public:
  static TRITONSERVER_Error* Create(
      ...
      ModelInstanceState** state){
        ...
        LOG_MESSAGE(
            TRITONSERVER_LOG_INFO,
            "ModelInstanceState Created success");
  ...
  int get_input_index(std::string input_name) {return input_name_map_.find(input_name)->second; }
  int get_output_index(std::string output_name) {return output_name_map_.find(output_name)->second; }
  void* get_d_input(std::string input_name) { return d_inputs_map.find(input_name)->second; }
  void* get_d_output(std::string output_name) { return d_outputs_map.find(output_name)->second; }
 ...
 private:
  ...
  std::map<std::string, void*> d_inputs_map; // Question: 没有手动在析构阶段释放指针，是否存在显存泄漏的风险？
...
ModelInstanceState::ModelInstanceState(
    ...
    std::string file_name = model_state_->RepositoryPath() + "/" + std::to_string(model_state_->Version()) + "/" + model_state_->ModelFileName();
    std::cout << file_name << std::endl;
    lightseq_model_ptr_ = std::shared_ptr<::lightseq::cuda::LSModel>(::lightseq::cuda::LSModelFactory::GetInstance().CreateModel(
        model_state->GetModelType(), file_name, model_state_->MaxBatchSize()));
    ...
    for(int idx = 0; idx < lightseq_model_ptr_->get_input_size(); idx ++) {
        ...
        TRITONSERVER_DataType data_type = model_state_->GetInputDataTypeByName(input_name);
    ...
    for(int idx = 0; idx < lightseq_model_ptr_->get_output_size(); idx ++) {
        ...
        TRITONSERVER_DataType data_type = model_state_->GetOutputDataTypeByName(output_name);
...
}}} // namespace triton::backend::lightseq