...
BiasAddTrans20314<T1, T2>::operator()(Variable* inp, Variable* bias) {
  ...
  Variable* res0 = new Variable(this->_name + "/res_0", trans_size * sizeof(T1),
                                trans_size * sizeof(T2));
  Variable* res1 = new Variable(this->_name + "/res_1", trans_size * sizeof(T1),
                                trans_size * sizeof(T2));
  Variable* res2 = new Variable(this->_name + "/res_2", trans_size * sizeof(T1),
                                trans_size * sizeof(T2));
...
void BiasAddTrans20314<T1, T2>::forward() {
  ...
  if (_context_ptr->built()) {
    ...
    print_vec(q_ptr, "after_transform res0", 10);
    print_vec(k_ptr, "after_transform res1", (k_ptr == nullptr) ? 0 : 10);
    print_vec(v_ptr, "after_transform res2", (v_ptr == nullptr) ? 0 : 10);
    printf("\n");
  }
#endif
}
